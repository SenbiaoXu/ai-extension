...
| 2026-03-07 | 外部消息功能修复 | 修复状态不更新问题(chat-response/stream-end/stream-error添加通知)，兼容OpenAI多内容格式消息 |
| 2026-03-09 | 模型列表转发功能 | /v1/models端点改为转发到浏览器扩展获取实际的端侧模型列表，支持动态模型发现 |
| 2026-03-09 | HTTP端口变更 | node-server默认HTTP端口从3000改为11435，避免与常见服务端口冲突 |
| 2026-03-09 | 权限优化 | 移除manifest.json中未使用的activeTab权限，遵循最小权限原则 |
| 2026-03-09 | 配置锁定与权限最小化 | API端点和密钥输入框改为只读，host_permissions最小化为localhost地址 |
| 2026-03-09 | 只读输入框样式优化 | 为只读输入框添加置灰样式，更新提示文字明确说明不可修改 |
| 2026-03-09 | host_permissions修复 | 移除端口号通配符，修复Chrome Web Store发布时报错问题 |
| 2026-03-09 | 连接状态显示优化 | 侧边栏连接状态新增"未配置端点"和"未选择模型"两种状态，解决显示"已连接"但无法对话的问题 |
| 2026-03-09 | 自动选择模型 | 侧边栏初始化时自动获取模型列表并选择第一个模型，解决首次使用时model为空的问题 |
| 2026-03-09 | 配置同步机制 | 侧边栏添加storage.onChanged监听器，配置页面修改模型后侧边栏自动更新配置 |
| 2026-03-10 | NPM包改造 | node-server改造为npm包`@hh-agent/local-model`，提供`hh-local-start`命令一键启动服务器 |
