...
**问题**: 侧边栏显示"已连接"，但发送消息时提示"请先在设置中配置API端点并选择模型"

**原因**: 连接测试逻辑与配置验证逻辑不一致
- `testConnection()` 只检查 `endpoint` 是否存在
- `hasValidConfig()` 要求同时有 `endpoint` 和 `model`

**解决**: 统一验证逻辑，增加更细粒度的状态显示：
1. `not-configured`: 未配置端点
2. `no-model`: 未选择模型
3. `connecting`: 连接中
4. `connected`: 已连接
5. `error`: 连接失败

**关键原则**: 状态显示应准确反映用户可执行的操作，避免误导用户

## 首次使用配置初始化

### 问题场景

**问题**: 默认配置中 `endpoint` 有值但 `model` 为空，首次打开侧边栏时显示"未选择模型"

**原因**: `DEFAULT_CONFIG` 中 `model: ''`，因为模型需要从API动态获取，无法预设

**解决**: 在侧边栏 `loadConfiguration()` 时自动检测并获取模型：
1. 检查 `endpoint` 存在但 `model` 为空的情况
2. 调用 `fetch-models` 获取可用模型列表
3. 自动选择第一个模型并保存配置
